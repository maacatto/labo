...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry)
{
  ...
  dataset_rf[ , entrenamiento := as.integer( (foto_mes== 202009 | (foto_mes>= 202010 &  foto_mes<= 202105)) & ( clase01==1 | azar < 0.10 )) ]
...
CanaritosAsesinos  <- function( canaritos_ratio=0.2 )
{
 ...
 # dataset[ , entrenamiento := foto_mes>= 202012 &  foto_mes<= 202103  & ( clase01==1 | azar < 0.10 ) ]
  dataset[ , entrenamiento := foto_mes>= 202007 &  foto_mes<= 202106  & ( clase01==1 | azar < 0.10 ) ]
  ...
  umbral  <- tb_importancia[ Feature %like% "canarito", median(pos) + 1.8*sd(pos) ]  #Atencion corto en la mediana mas DOS desvios!!
...
dt <- dataset[sample(nrow(dataset), 3), ]
fwrite( dt,
