...
#STACKING validation
PARAM$semillas_azar  <- c( 807299, 962041, 705689, 909463, 637597, 503963 )
PARAM$stacking$entrenar <- TRUE #si volver a entrenar o leer desde archivo
...
stack_3lgbm_glm <- function(dttrain, dvalid, params1,params2,params3, semillas_azar , verbose = TRUE) {
  ...
  tb_prediccion  <- dvalid[  , list( numero_de_cliente) ]
...
#  model_lgb3 <- vector("list", nfolds)
  ...
  x_glm <- matrix(NA, nrow = nrow(dtvalid), ncol = 2L)
  ...
  for (i in 1L:nfolds) {
    ...
    model_lgb1[[i]] <- fit
    prediccion <- predict(fit, data.matrix( dtvalid ))
    tb_prediccion[ , prob := prediccion ]
    tb_prediccion[ , rank := frank(-prob, ties.method = "random")] #RANK
    
    x_glm[, "lgb1"] <- data.matrix(tb_prediccion[,rank])
  ...
  for (i in 1L:nfolds) {
    ...
    x_glm[, "lgb2"] <- data.matrix(tb_prediccion[,rank])
    #borro y limpio la memoria para la vuelta siguiente del for
    rm( dtrain )
    gc()
  }
