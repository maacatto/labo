...
fwrite( dataset_b2,
        file= paste0("cluster_de_bajas_",n_clus,"clusters.txt"),
        sep= "\t" )
...
fwrite( data.table(all), 
        file= paste0("cluster_",n_clus,"clusters_summary.txt"),
        sep= "," )
...
all <- list()
for (i in campos_buenos) {
  all <- rbind(all, list(i, t(dataset[  , mean(get(i)) ]) ) )
}
fwrite( data.table(all), 
...
#quito los nulos para que se pueda ejecutar randomForest,  Dios que algoritmo prehistorico ...
dataset12  <- na.roughfix( dataset12[,..col_buenas] )
...
all <- list()
for (i in campos_buenos) {
  all <- rbind(all, list(i, t(dataset12[  , mean(get(i)),  cluster2 ][,2]) ,t(dataset12[  , mean(get(i))])  ) )
}
fwrite( data.table(all), 
        file= paste0("cluster_",n_clus,"clusters_summary_12meses.txt"),
        sep= "," )
