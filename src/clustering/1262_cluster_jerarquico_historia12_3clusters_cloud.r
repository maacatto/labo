...
fwrite( dataset,
        file= "cluster_de_bajas_4clusters.txt",
        sep= "\t" )
...
for (i in campos_buenos){
  fig1 <- ggplot(dataset, aes(x = cluster2, group = cluster2, y=get(i),  color = cluster2)) +
    geom_boxplot() +
    theme_bw() +
    coord_cartesian(ylim = c( (dataset[  , quantile(get(i),0.25)] - 2 * dataset[  , IQR(get(i))]), dataset[  , quantile(get(i),0.75)] + 2 * dataset[  , IQR(get(i))] )) +
    ggtitle(i)
  print(fig1)
}
dev.off()
...
all <- list()
for (i in campos_buenos) {
  all <- rbind(all, list(i, t(dataset[  , mean(get(i)),  cluster2 ][,2]) ))
}
