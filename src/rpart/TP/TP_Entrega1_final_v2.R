...
ArbolSimple_binaria  <- function( fold_test, data, param )
{
  param2 <- param
...
ArbolesCrossValidation  <- function( data, cols, param, qfolds, pagrupa, semilla )
{
  ...
  ganancias  <- mcmapply( ArbolSimple_binaria, 
...
#'------------------------------------------------------------------------------
  ...
  # dtrain[, paste0("n_rank_", var.monet$campo) := lapply(.SD, frank),
  #        .SDcols = var.monet$campo]
  # dtrain[, var.monet$campo :=NULL] #remuevo las cols monetarias dejo las ranking
  # 
  # #rankeo en entrenamiento
  # dapply[, paste0("n_rank_", var.monet$campo) := lapply(.SD, frank),
  #        .SDcols = var.monet$campo]
...
# dapply$numero_de_cliente = dapply.numero_de_cliente
  ...
  columnas <- "clase_binaria ~ . -clase_ternaria -n_rank_mcomisiones_mantenimiento -Master_mpagominimo -Visa_mpagominimo"
  #columnas <- "clase_binaria ~ . -clase_ternaria -mcomisiones_mantenimiento"
...
#                        minbucket=  156,   # 309
#                        maxdepth=     12 )  #  12
  # hyperparametros <- list(cp=          -0.999907609,#  -0.89
  #                        minsplit=  666,   # 621
  #                        minbucket=  325,   # 309
  #                        maxdepth=     14 )  #  12
  #-0.999907609	666	325	14
